...
class _Project:
    ...
    def parseETree(self, eTree):
        ...
        if self.root.tag == "projects":
            raise TypeError(
                "XML does not describe a single project, but a collection of projects."
            )
...
class _Issue:
    ...
    def parseETree(self, eTree):
        ...
        if self.root.tag == "issues":
            raise TypeError(
                "XML does not describe a single issue, but a collection of issues."
            )
        ...
        self.author = self.root.find("author").attrib
        self.priority = self.root.find("priority").attrib
        self.fixed_version = (
            self.root.find("fixed_version").attrib
            if self.root.find("fixed_version") is not None
            else {"name": "-"}
        )
        ...
        try:
            self.assigned_to = (
                self.root.find("assigned_to").attrib
                if self.root.find("assigned_to") is not None
                else {"name": "N/A"}
            )
        except:
            pass
        ...
        if self.root.find("children") is not None:
            for field in self.root.find("children"):
                self.children.append(
                    {"id": field.attrib["id"], "subject": field.find("subject").text}
                )

        if self.root.find("attachments") is not None:
            for attachment in self.root.find("attachments"):
                self.attachments.append(
                    {
                        "id": attachment.attrib.get("id"),
                        "filename": attachment.find("filename").text,
                        "description": attachment.find("description").text,
                        "author": attachment.find("author").attrib.get("name"),
                        "created_on": attachment.find("created_on").text,
                        "url": (attachment.find("content_url").text).replace("http://localhost:3000", settings.URL),
                     }
                )
                
        if self.root.find("journals") is not None:
            for nid, field in enumerate(self.root.find("journals")):
                j_entry = {
                    "author": field.find("user").attrib,
                    "note_number": nid,
                    "created_on": parse_redmine_date(field.find("created_on").text),
                    "notes": field.find("notes").text,
                    "details": [],
                }
                if field.find("details") is not None:
                    details = []
                    for d in field.find("details"):
                        detail = {
                            "property": d.attrib["property"] if d.attrib["property"] is not None else "",
                            "name": d.attrib["name"],
                            "old": d.find("old_value").text,
                            "new": d.find("new_value").text,
                        }
...
class _TimeEntry:
    ...
    def parseETree(self, eTree):
        ...
        if self.root.tag == "time_entries":
            raise TypeError(
                "XML does not describe a single timeentry, but a collection of timeentries."
            )
...
class Redmine:
    ...
    def getVersions(self, projectIdent):
        ...
        versions = [
            self.Version(ET.ElementTree(version))
    ...
    def getIssue(self, issueID):
        ...
        return self.Issue(
            self.get(
                "issues/" + str(issueID) + ".xml",
                parms={"include": "journals,children,attachments"},
            )
        )
    ...
    def findIssues(self, get_all_info=False, **kwargs):
        ...
        issues = self.get(
            "issues.xml?%s" % "&".join(["%s=%s" % (k, v) for k, v in kwargs.items()])
        )

        if get_all_info:
            return [
                self.getIssue(self.Issue(ET.ElementTree(issue)).id)
                for issue in issues.findall("issue")
            ]
        else:
            return [
                self.Issue(ET.ElementTree(issue)) for issue in issues.findall("issue")
            ]

    def timesheets(self, projectIdent, **kwargs):
        times = self.get("time_entries.xml")
        return [
            self.TimeEntry(ET.ElementTree(timeentry))
            for timeentry in times.findall("time_entry")
        ]
